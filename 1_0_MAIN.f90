...
!	*Subroutine	LEVAR !Subroutine for reading the file with fixed parameters (ALBEDOS, IAF, RS, Z; monthly) (File: ALBIAF.FIX)
...
!  Call subroutines related to input files that are readen only once reading and MGB-IPH variables allocation
	...
	CALL LEVAR !Subroutine for reading the file with fixed parameters (ALBEDOS, IAF, RS, Z; monthly) (File: ALBIAF.FIX)
...
!*****************************************************************
	...
	IND_FLOODROUTING = 2 !MS: TODINI  (1)INERTIAL (2)TODINI
	...
	!MS: TODINI
    ELSE IF (IND_FLOODROUTING==2) THEN 
        WRITE(*,*)'-----------------'
        WRITE(*,*)' RUNNING TODINI '
        WRITE(*,*)'-----------------'
        HDFLAG = 0       !MATA QQ CONFIGURACAO DE HIDRODINAMICO
        FLAG_TODINI = 1  !RODA COM PLANICIE "INATIVA"
        !FLAG_TODINI = 2  !RODA COM PLANICIE ATIVA
        CALL LEFLOODPLN  !VAI UTILIZAR INFORMACAO DAS MESMAS TABELAS DO INERCIAL!
    ELSE
        hdFLAG = 0
    ENDIF
    hdFLAG0=sum(hdFLAG) 
...
!  Call subroutines that calculates MicroBasins, rivers channels, and floodplains characteristics
    ...
    CALL INTECLIM ! Assign nearest climate station
...
!  Openning of MGB-IPH Input and Output files that are read and written during the simulation
    WRITE(*,*)'Openning input/output files...'
    WRITE(FILLOG,*)'Openning input/output files...'
	...
	OPEN(FILAJU,FILE=OUTPUT_DIRECTORY // 'AJUSTE_corr.fob',STATUS='UNKNOWN') !Output file with objective functions valuess for the locations with observed data
    OPEN(FILAJU2,FILE=OUTPUT_DIRECTORY // 'AJUSTE.fob',STATUS='UNKNOWN') !Output file with objective functions valuess for the locations with observed data
...
!  Informs the model general setup
    ...
    WRITE(FILLOG,*)'ICALIB (simulation=0; autocalibration=1; forecasting=2)'
    ...
    WRITE(*,'(A50,I8)')'(simulation=0; autocalibration=1; forecasting=2)'
    if (IND_FLOODROUTING == 0) then
        WRITE(*,'(A42)')'Flood routing method: Muskingum-Cunge'
    end if
    if (IND_FLOODROUTING == 1) then
        WRITE(*,'(A30)')'Flood routing method: Inertial'
        WRITE(*,'(A30,F4.2)')'Alpha: ', ALPHA
    end if
    WRITE(*,*)'PLEASE VERIFY INPUT INFORMATION AND PRESS ENTER TO START RUN...'
    WRITE(*,*)''

!  Select the mode of MGB-IPH run: Simulation (ICALIB=0), Automatic Calibration (ICALIB=1)
